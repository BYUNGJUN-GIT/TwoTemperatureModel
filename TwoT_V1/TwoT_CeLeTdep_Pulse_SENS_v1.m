...
[~,TE_ref] = TwoT_CeLeTdep_Pulse_v1(dx,nx,LambdaL,LambdaE,CL,gammaE,T0,Gll,gEL,...
...
for j = 1:length(gEL)
    ...
    if gEL(j) > 1
        ...
        [~,TE_temp] = TwoT_CeLeTdep_Pulse_v1(dx,nx,LambdaL,LambdaE,CL,gammaE,T0,Gll,gEL_temp,...
            tdelay_model,delta_time,nt,heatL,heatE,PulseTemp);
...
for j = 1:length(LambdaL)
    ...
    if LambdaL(j) > 0.01
        ...
        [~,TE_temp] = TwoT_CeLeTdep_Pulse_v1(dx,nx,LambdaL_temp,LambdaE,CL,gammaE,T0,Gll,gEL,...
            tdelay_model,delta_time,nt,heatL,heatE,PulseTemp);
...
for j = 1:length(LambdaE)
    ...
    if LambdaE(j) > 0.01
        ...
        [~,TE_temp] = TwoT_CeLeTdep_Pulse_v1(dx,nx,LambdaL,LambdaE_temp,CL,gammaE,T0,Gll,gEL,...
            tdelay_model,delta_time,nt,heatL,heatE,PulseTemp);
...
for j = 1:length(CL)
    ...
    if CL(j) > 1
        ...
        [~,TE_temp] = TwoT_CeLeTdep_Pulse_v1(dx,nx,LambdaL,LambdaE,CL_temp,gammaE,T0,Gll,gEL,...
            tdelay_model,delta_time,nt,heatL,heatE,PulseTemp);
...
for j = 1:length(gammaE)
    ...
    if gammaE(j) > 0.01
        ...
        [~,TE_temp] = TwoT_CeLeTdep_Pulse_v1(dx,nx,LambdaL,LambdaE,CL,gammaE_temp,T0,Gll,gEL,...
            tdelay_model,delta_time,nt,heatL,heatE,PulseTemp);
        S_gammaE(:,j) = log_ratio(TE_temp(:,IndFit), TE_fit)/log(1.01);
    end
end

S_Gll = zeros(length(tdelay_model),length(Gll));
for j = 1:length(Gll)
    Gll_temp = Gll;
    if Gll(j) > 0
        Gll_temp(j) = Gll_temp(j)*1.01;
        [~,TE_temp] = TwoT_CeLeTdep_Pulse_v1(dx,nx,LambdaL,LambdaE,CL,gammaE,T0,Gll_temp,gEL,...
            tdelay_model,delta_time,nt,heatL,heatE,PulseTemp);
        S_Gll(:,j) = log_ratio(TE_temp(:,IndFit), TE_fit)/log(1.01);
    end
end
